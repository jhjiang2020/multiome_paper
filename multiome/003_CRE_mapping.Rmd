...
DORC_TF_matrix_all <- mp_FigR %>% 
  ...
  as.matrix
saveRDS(DORC_TF_matrix_all, "~/Oxford/Results/multiome/rdata/DORC_TF_matrix.rds")
```
...
CAD_genes <- read.csv("~/Oxford/Results/multiome/multiome_share/003_CAD_GWAS/archive/CAD_overlapping_links(CREs)_annotated.csv") %>% select(CRE_gene) %>% unique %>% unlist
...
saveRDS(bg, "~/Oxford/Results/multiome/multiome_share/rdata/bgPeaks.rds")
```

## eQTL
Whole blood eQTL SNPs were downloaded from GTEx v8 and eQTL gene and manually lifted over to hg38
```{r}
eqtl.df <- readRDS("~/Oxford/Results/multiome/multiome_share/rdata/annot/eqtl.df.hg38.rds")
...
```{r}
ABC_links_mono.gr <- readRDS("~/Oxford/Results/multiome/rdata/ABC_links_mono.rds")
...
```{r}
ch <- "../../hg19ToHg38.over.chain"
phic.gr.hg38<- read.table("~/Oxford/Results/multiome/TAD_definitions/PCHiC_peak_matrix_cutoff5.tsv", header = T) %>% 
  filter(oeName != ".") %>%
